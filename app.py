...
st.set_page_config(page_title="CareerPath Navigator",
                   layout="wide",
...
def render_welcome_tab():
    ...
    **CareerPath Navigator** is designed to bridge the gap between 
    ...
    st.write(
        "If you want to start fresh or remove your personal data, you can reset the application data here."
    )

    reset_col1, reset_col2 = st.columns([3, 1])
    with reset_col1:
        st.warning(
            "This will clear all your skills, uploaded files, and saved learning plans. This cannot be undone."
        )
    with reset_col2:
        if st.button("Reset All Data"):
            ...
            # Refresh the page
            st.success(
                "All data has been reset! The page will refresh momentarily.")
            st.rerun()
    ...
    if prompt := st.chat_input(
            "What would you like to know about your career options?"):
            ...
            try:
                ...
                if api_key:
                    ...
                    messages = [{
                        ...
                        "system",
                        "content":
                        """You are a helpful career assistant in the CareerPath Navigator application. 
...
def render_portfolio_tab():
    ...
    st.write(
        "Upload and organize your projects to showcase your skills and experience."
    )
    ...
    with upload_tab:
        ...
        with st.form("project_form", clear_on_submit=True):
            project_name = st.text_input(
                "Project Name", placeholder="e.g., E-commerce Website")
            project_desc = st.text_area(
                "Project Description",
                placeholder="Describe your project and your role...",
                height=150)
            project_skills = st.text_input(
            ...
            if submitted:
                if not project_name or not project_desc:
                    st.error(
                        "Please provide at least a project name and description."
                    )
                else:
                    ...
                    new_project = {
                        ...
                        "skills": [
                            skill.strip()
                            for skill in project_skills.split(",")
                            if skill.strip()
                        ],
                        "link":
                    ...
                    if "user_skills" in st.session_state and new_project[
                            "skills"]:
                        for skill in new_project["skills"]:
                            if skill not in st.session_state.user_skills:
                                ...
                                if "projects" not in st.session_state.user_skills[
                                        skill]:
                                    st.session_state.user_skills[skill][
                                        "projects"] = []

                                if project_name not in st.session_state.user_skills[
                                        skill]["projects"]:
                                    st.session_state.user_skills[skill][
    ...
    with view_tab:
        if not st.session_state.projects:
            st.info(
                "No projects added yet. Add some projects in the 'Add Project' tab."
            )
        else:
            ...
            for i, project in enumerate(st.session_state.projects):
                with st.expander(
                        f"{project['name']} ({project['date_added']})"):
                    ...
                    if project['link']:
                        st.markdown(
                            f"**Link:** [{project['link']}]({project['link']})"
                        )
                    ...
                    if project['file'] is not None:
                        ...
                        if project['file_type'] and 'image' in project[
                                'file_type']:
                            st.image(project['file'],
                                     caption=f"Image for {project['name']}")
                        elif project['file_type'] and 'pdf' in project[
                                'file_type']:
...
def render_find_pathway_tab():
    ...
    for metric_id in metrics:
        ...
        with col1 if metric_count % 2 == 0 else col2:
            ...
            importance = st.slider("How important is this to you?",
                                   1,
                                   10,
                                   5,
    ...
    if st.button("Find Matching Pathways", key="find_pathways_btn"):
        matches = calculate_pathway_matches(pathways_df, user_preferences,
                                            importance_weights)
        ...
        for pathway_id, score, explanation in matches[:5]:
            pathway = next((p for p in pathways_df if p["id"] == pathway_id),
                           None)
            if pathway:
...
def main():
    ...
    with st.sidebar:
        ...
        if is_authenticated():
            ...
            st.markdown(
                f"<div style='text-align: center'><a href='{auth_url}' style='background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; margin: 10px 0;'>Login with Replit</a></div>",
    ...
    tabs = st.tabs([
        "Welcome", "Skill Graph", "Project Portfolio", "2×2 Matrix",
        "Find Your Pathway", "AI Roadmap", "Interview Coach", "Job Tracker",
        "About"
    ])
    ...
    with tabs[3]:
        ...
        st.write(
            "This premium feature helps you compare different career paths based on key metrics."
        )
    ...
    with tabs[4]:
        ...
        st.write(
            "This premium feature matches your preferences and skills with optimal career paths."
        )
        st.write("Premium features coming soon!")
    ...
    with tabs[5]:
        ...
        st.write(
            "This premium feature generates personalized career development plans and guidance."
        )
        st.write("Premium features coming soon!")
    ...
    with tabs[6]:
        ...
        st.write(
            "This premium feature helps you prepare for interviews using AI-powered practice sessions tailored to your target roles."
        )
        st.write("Premium features coming soon!")
