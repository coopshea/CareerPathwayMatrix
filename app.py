...
# Job & Resume Analysis tab - now tab2
with tab2:
...
# Project Portfolio tab - now correctly placed in tab5
with tab5:
...
with tab6:
    ...
    # Only show sidebar controls in this tab
    with st.sidebar:
        ...
        # Select x and y axis metrics with defaults focused on technical specialization and success probability
        x_metric = st.selectbox(
            "X-Axis Metric", 
            list(metrics_data.keys()),
            index=list(metrics_data.keys()).index("technical_specialization"),
            key="x_axis_metric_tab6"
        )

        y_metric = st.selectbox(
            "Y-Axis Metric", 
            list(metrics_data.keys()),
            index=list(metrics_data.keys()).index("success_probability"),
            key="y_axis_metric_tab6"
        )
    ...
    # Create the visualization using the filtered pathways and selected metrics
    ...
    if st.session_state.selected_pathway is None:
        ...
        with col2:
            ...
            with roadmap_cols[0]:
                if st.button("Generate Basic Roadmap", key="gen_basic_tab6"):
                    st.session_state.generate_roadmap_for = pathway_details
                    # Switch to the AI roadmap tab (index 3)
                    st.session_state.active_tab = 3  # Basic roadmap is now tab3
                    st.rerun()
            with roadmap_cols[1]:
                if st.button("Generate AI-Powered Roadmap", key="gen_ai_tab6"):
                    st.session_state.generate_roadmap_for = pathway_details
                    # Switch to the AI roadmap tab (index 3)
                    st.session_state.active_tab = 3  # AI roadmap is now tab3
                    st.rerun()
    ...
    if st.button("Find Matching Pathways"):
        ...
        for i, (pathway_id, score, explanation) in enumerate(matches[:3]):
            ...
            if st.button(f"View Detailed Pathway", key=f"view_{i}"):
                ...
                st.session_state.active_tab = 6  # 2x2 Matrix tab is now tab6
                st.rerun()
