...
def create_selectbox(label, options, default_index=0, tab_name="ai_roadmap", widget_name="selector", format_func=None):
    ...
    return st.selectbox(
        ...
        key=f"{tab_name}_{widget_name}_selectbox"
    )

def create_file_uploader(label, file_types, tab_name="ai_roadmap", widget_name="uploader"):
    """Create a file uploader with a unique key based on tab and widget name"""
    return st.file_uploader(
        label,
        type=file_types,
        key=f"{tab_name}_{widget_name}_uploader"
    )
...
def create_questionnaire():
    ...
    responses["work_environment"] = st.radio(
        ...
        key="work_environment_radio_questionnaire"
    )

    responses["management_preference"] = create_selectbox(
        "Do you prefer being a manager or an individual contributor?",
        ["Individual contributor", "Manager", "Both/Either"],
        tab_name="questionnaire",
        widget_name="management_preference"
    )
    ...
    responses["timeframe"] = create_selectbox(
...
def render_ai_roadmap_tab(user_data=None, pathways_df=None, metrics_data=None, selectbox=None, file_uploader=None):
    """
    Render the AI roadmap tab with the provided user data
    
    Args:
        user_data: The user data object
        pathways_df: The pathways dataframe
    ...
    if user_data and user_data.selected_pathway:
        pathway_id = user_data.selected_pathway
...
def ai_roadmap_generator_page(pathway=None, pathways_df=None, metrics_data=None):
    ...
    # No dream job functionality
    use_dream_job = False

    if not use_dream_job:
        if pathway is None and pathways_df is not None:
            # Let user select a pathway using our utility function
            pathway_options = pathways_df["name"].tolist()
            selected_pathway_name = create_selectbox(
                "Select a career pathway:", 
                pathway_options,
                tab_name="ai_roadmap_main",
                widget_name="pathway_select"
            )
    ...
    with input_tab3:
        ...
        def create_combined_questionnaire():
            ...
            responses["management_preference"] = create_selectbox(
                ...
                tab_name="combined_questionnaire",
                widget_name="management_preference"
            )
            ...
            if generate_button3:
                with st.spinner("Creating your comprehensive roadmap from current skills to dream job..."):
                    ...
                    roadmap = generate_ai_roadmap(pathway, {
                        ...
                        "career_goal": questionnaire_responses_combined.get("career_goals", "Career advancement")
                    }, metrics_data)
